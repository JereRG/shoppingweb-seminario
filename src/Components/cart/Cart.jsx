...
const Cart = ({ cart, removeFromCart, updateQuantity, setCurrentPage }) => {
    return (
        <Box sx={{ p: 2, ml: 1, mr: 1, minHeight: "100vh" }}>
                        ...
                        {cart.map((product) => (
                            <Grid item xs={12} sm={6} md={4} lg={3} key={product.id}>
                                <Box sx={{ border: '1px solid #ddd', p: 2, position: 'relative', borderRadius: 2, bgcolor: 'white', display: 'flex',
                                    ...
                                    <img src={product.image} alt={product.title} style={{ width: '100%', height: 'auto', maxHeight: 150, borderRadius: '5px', objectFit: 'cover',  }} />
